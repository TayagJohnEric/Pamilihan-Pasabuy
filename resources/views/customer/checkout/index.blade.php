...
<div class="min-h-screen bg-gray-50 pb-24 lg:pb-8">
    <form id="rider-search-form" method="GET" action="{{ route('checkout.index') }}"></form>
    <div class="max-w-[90rem] mx-auto ">
        <!-- Page Header -->
        <div class="py-4 lg:py-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Checkout</h1>
        ...
        <!-- Checkout Form -->
        <form id="checkout-form" method="POST" action="{{ route('checkout.process') }}" class="space-y-4 lg:space-y-8">
            @csrf
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-8">
                ...
                <div class="lg:col-span-8 space-y-4 lg:space-y-8">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
                        <h2 class="text-lg sm:text-xl font-semibold text-gray-900 mb-4 sm:mb-6 flex items-center">
                            <svg class="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                            </svg>
                            <span class="truncate">Order Items ({{ $itemCount }})</span>
                        </h2>
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            @foreach($cartItems as $item)
                                ...
                                <div class="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-200">
                                    <!-- Product Image -->
                                    <div class="flex-shrink-0">
                                        @if($item->product->image_url)
                                            <img src="{{ asset('storage/' . $item->product->image_url) }}"
                                                 alt="{{ $item->product->product_name }}" 
                                                 class="w-14 h-14 sm:w-20 sm:h-20 object-cover rounded-lg border border-gray-200">
                                        @else
                                            <div class="w-14 h-14 sm:w-20 sm:h-20 bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                                    ...
                                    <div class="flex-grow min-w-0">
                                        <h4 class="font-semibold text-sm sm:text-base text-gray-900 truncate">{{ $item->product->product_name }}</h4>
                                        <p class="text-xs sm:text-sm text-gray-600 truncate">by {{ $item->product->vendor->vendor_name }}</p>
                                        ...
                                        @if($isBudgetBased)
                                            <div class="mt-2">
                                                ...
                                                @if($item->customer_notes)
                                                    <p class="text-xs text-gray-600 mt-1 line-clamp-1">Notes: {{ $item->customer_notes }}</p>
                                                @endif
                                            </div>
                                        @else
                                        ...
                                        <!-- Price on mobile (below product info) -->
                        ...
                        @if($hasBudgetItems)
                            <div class="mt-4 sm:mt-6 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div class="flex items-start gap-2 sm:gap-3">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                                    </svg>
                                    <div>
                                        <p class="text-xs sm:text-sm font-medium text-blue-800">Budget Items Notice</p>
                                        <p class="text-xs sm:text-sm text-blue-700 mt-1">Some items are budget-based. The vendor will confirm final quantity and pricing.</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
                                ...
                                @foreach($savedAddresses as $address)
                                    <div class="relative">
                                        ...
                                        <label for="address_{{ $address->id }}" 
                                               class="flex items-start p-3 sm:p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all">
                                            <div class="flex-grow min-w-0">
                                                <div class="flex items-center justify-between mb-2 gap-2">
                                                    <h4 class="font-medium text-sm sm:text-base text-gray-900 truncate">{{ $address->address_label }}</h4>
                                                ...
                                                <p class="text-xs sm:text-sm text-gray-600 mb-1">{{ $address->address_line1 }}</p>
                                                <p class="text-xs sm:text-sm text-gray-500">{{ $address->district->name }}</p>
                                                @if($address->delivery_notes)
                                                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">Notes: {{ $address->delivery_notes }}</p>
                                                @endif
                                                <p class="text-xs sm:text-sm font-medium text-blue-600 mt-2">
                        ...
                        @else
                            <div class="text-center py-8">
                                <svg class="w-10 h-10 sm:w-12 sm:h-12 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                                </svg>
                                <p class="text-sm sm:text-base text-gray-600 mb-4">No saved addresses found.</p>
                                <a href="{{ route('customer.saved_addresses.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
                        <h2 class="text-lg sm:text-xl font-semibold text-gray-900 mb-4 sm:mb-6 flex items-center">
                            ...
                            <span class="truncate">Payment Method</span>
                        </h2>
                            ...
                            @foreach($paymentMethods as $value => $label)
                                <div class="relative">
                                    ...
                                    <label for="payment_{{ $value }}" 
                                           class="flex items-center p-3 sm:p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all">
                                        <div class="flex items-center gap-3 sm:gap-4 w-full">
                                            @if($value === 'cod')
                                                <div class="w-8 h-8 sm:w-10 sm:h-10 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0">
                                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-green-600" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <h4 class="font-medium text-sm sm:text-base text-gray-900">{{ $label }}</h4>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
                        <h2 class="text-lg sm:text-xl font-semibold text-gray-900 mb-4 sm:mb-6 flex items-center">
                            ...
                            <span class="truncate">Rider Selection</span>
                        </h2>
                            ...
                            <div class="space-y-3">
                                    ...
                                    <label for="system_assign" 
                                        ...
                                        <div class="flex items-center gap-3 sm:gap-4 w-full">
                                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0">
                                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                                                </svg>
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4 class="font-medium text-sm sm:text-base text-gray-900">Assign One For Me</h4>
                                                <p class="text-xs sm:text-sm text-gray-600">System finds an available rider automatically</p>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                ...
                                <div class="relative">
                                    ...
                                    <label for="choose_rider" 
                                        ...
                                        <div class="flex items-center gap-3 sm:gap-4 w-full">
                                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0">
                                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/>
                                                </svg>
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4 class="font-medium text-sm sm:text-base text-gray-900">Choose My Rider</h4>
                                                <p class="text-xs sm:text-sm text-gray-600">Select from available riders</p>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                        ...
                        <div id="rider-selection-list" class="hidden">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <h3 class="text-base sm:text-lg font-medium text-gray-900">Available Riders</h3>
                            </div>

                            <div class="mt-3 mb-4" id="rider-search-controls">
                                <div class="flex flex-col gap-2 sm:flex-row">
                                    ...
                                    <div class="flex gap-2">
                                        @if($riderSearchTerm)
                                            <a href="{{ route('checkout.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                                                Clear
                                            </a>
                                        @endif
                                        <button type="submit" form="rider-search-form" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
                                            Search
                                        </button>
                                    </div>
                                </div>
                            ...
                            @if($riderSearchTerm)
                                <p class="text-xs sm:text-sm text-gray-600 mb-3">
                                    Showing <span class="font-semibold text-gray-900">{{ $availableRiders->count() }}</span> result{{ $availableRiders->count() === 1 ? '' : 's' }} for
                                    <span class="font-semibold text-gray-900">"{{ $riderSearchTerm }}"</span>
                                </p>
                            @endif
                                            ...
                                            <label for="rider_{{ $rider->id }}" 
                                                ...
                                                <div class="flex items-center gap-3 w-full">
                                                    ...
                                                    <div class="flex-shrink-0">
                                                        ...
                                                        @else
                                                            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-gray-200 rounded-full flex items-center justify-center border-2 border-gray-200">
                                                                <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gray-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                                        ...
                                                        <div class="flex items-center flex-wrap gap-x-2 gap-y-1 mt-1">
                                                            @if($rider->rider->average_rating)
                                                                <div class="flex items-center">
                                                                    <svg class="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 mr-1" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                                    </svg>
                                                                    <span class="text-xs sm:text-sm text-gray-600">{{ number_format($rider->rider->average_rating, 1) }}</span>
                                                                </div>
                                                            @endif
                            ...
                            @else
                                <div class="text-center py-6">
                                    ...
                                    @if($riderSearchTerm)
                                        <p class="text-sm sm:text-base text-gray-600">No riders matched your search.</p>
                                        <p class="text-xs sm:text-sm text-gray-500 mt-1">Try different keywords or <a href="{{ route('checkout.index') }}" class="text-blue-600 hover:underline">clear the search</a>.</p>
                                    @else
                                        <p class="text-sm sm:text-base text-gray-600">No riders are currently available.</p>
                                        <p class="text-xs sm:text-sm text-gray-500 mt-1">Please try "Assign One For Me" or try again later.</p>
                                    @endif
                                </div>
                            @endif
                        </div>
                ...
                <!-- Order Summary Column (Desktop: Right Sidebar, Mobile: Fixed Bottom) -->
                        ...
                        <div class="px-4 py-3">
                            ...
                            <button type="button" 
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="text-lg font-bold text-gray-900" id="mobile-total-amount">
                                        @if($savedAddresses->count() > 0)
                                            ₱{{ number_format($subtotal + $savedAddresses->first()->district->delivery_fee, 2) }}
                                        @else
                                            ₱{{ number_format($subtotal, 2) }}
                                    ...
                                    <svg id="summary-chevron" class="w-5 h-5 text-gray-400 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            <!-- Expandable Details -->
                            <div id="mobile-summary-details" class="hidden mb-3 pt-3 border-t border-gray-200 space-y-2">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Items ({{ $itemCount }})</span>
                                    <span class="font-medium text-gray-900">₱{{ number_format($subtotal, 2) }}</span>
                                </div>
                                ...
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Delivery Fee</span>
                                    <span class="font-medium text-gray-900" id="mobile-delivery-fee">
                                        @if($savedAddresses->count() > 0)
                                            ₱{{ number_format($savedAddresses->first()->district->delivery_fee, 2) }}
                                        @else
                                            ₱0.00
                                ...
                                @if($hasBudgetItems)
                                    <div class="flex items-start gap-2 p-2 bg-blue-50 border border-blue-200 rounded">
                                        <svg class="w-4 h-4 text-blue-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                                        </svg>
                                        <p class="text-xs text-blue-700">Final total may vary based on vendor confirmation</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('cart.index') }}" 
                                   class="flex-1 border-2 border-gray-300 text-gray-700 px-4 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors inline-flex items-center justify-center text-sm">
                                ...
                                <button type="submit" 
                                        ...
                                        class="flex-[2] bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-4 py-3 rounded-lg font-semibold hover:from-emerald-700 hover:to-teal-700 transition-all inline-flex items-center justify-center text-sm">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    <span id="mobile-submit-btn-text">Place Order</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#mobile-summary-toggle').on('click', function() {
        const details = $('#mobile-summary-details');
    ...
    // Check if there's a search term and auto-select "Choose My Rider"
    const hasSearchTerm = "{{ $riderSearchTerm }}" !== "";
    if (hasSearchTerm) {
        $('#choose_rider').prop('checked', true).trigger('change');
    }

    // Handle rider search form separately so it won't trigger checkout validation
    $('#rider-search-form').on('submit', function(e) {
        e.stopPropagation();
    });
    ...
    function updateDeliveryFee(deliveryFee) {
        ...
        $('#mobile-delivery-fee').text('₱' + deliveryFee.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
        $('#mobile-total-amount').text('₱' + total.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
    }
    ...
    function updateSubmitButtonText() {
        ...
        const mobileSubmitBtnText = $('#mobile-submit-btn-text');
        
        if (paymentMethod === 'online_payment') {
            submitBtnText.text('Proceed to Payment');
            mobileSubmitBtnText.text('Proceed to Payment');
        } else {
            submitBtnText.text('Place Order');
            mobileSubmitBtnText.text('Place Order');
        }
    }
    ...
    $('#checkout-form').on('submit', function(e) {
        ...
        const mobileSubmitBtn = $('#mobile-checkout-submit-btn');
        submitBtn.prop('disabled', true).addClass('opacity-75 cursor-not-allowed');
        mobileSubmitBtn.prop('disabled', true).addClass('opacity-75 cursor-not-allowed');
        $('#submit-btn-text').text(paymentMethod === 'online_payment' ? 'Redirecting...' : 'Processing...');
        $('#mobile-submit-btn-text').text(paymentMethod === 'online_payment' ? 'Redirecting...' : 'Processing...');
    ...
    // Trigger the initial state based on default selection
    $('input[name="rider_selection_type"]:checked').trigger('change');
});
</script>
@endsection