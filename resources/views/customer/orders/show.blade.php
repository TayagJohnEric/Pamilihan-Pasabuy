                ...
                <div class="flex flex-wrap gap-3 mt-4">
                    @if($order->status === 'assigned' && $order->payment_method === 'online_payment' && $order->payment_status === 'pending')
                        @php
                            ...
                            $isRejected = $payment && $payment->rider_verification_status === 'rejected';
                            $isPendingVerification = $payment && $payment->rider_verification_status === 'pending' && $hasSubmittedPayment;
                        @endphp
                        
                        @if($isPendingVerification)
                            <!-- Waiting for Rider Approval -->
                            <div class="w-full p-4 bg-blue-50 border border-blue-200 rounded-xl mb-4">
                                <div class="flex items-start gap-3">
                                    <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 animate-pulse" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
                                    </svg>
                                    <div class="flex-1">
                                        <h3 class="text-sm font-semibold text-blue-800 mb-1">Payment Proof Submitted</h3>
                                        <p class="text-sm text-blue-700">Your payment proof is being verified by the rider. You'll be notified once confirmed.</p>
                                    </div>
                                </div>
                            </div>
                        @elseif($isRejected)
                            <!-- Payment Rejected - Show Button Again -->
                            <div class="w-full p-4 bg-red-50 border border-red-200 rounded-xl mb-4">
                                <div class="flex items-start gap-3">
                                    <svg class="w-6 h-6 text-red-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                                    </svg>
                                    <div class="flex-1">
                                        <h3 class="text-sm font-semibold text-red-800 mb-1">Payment Verification Issue</h3>
                                        <p class="text-sm text-red-700 mb-2">The rider has not confirmed your payment. Reason: {{ $payment->rider_verification_notes ?? 'Please resubmit your payment proof.' }}</p>
                        ...
                        @else
                            <!-- Payment Required Button (No payment submitted yet) -->
